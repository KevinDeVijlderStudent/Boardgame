...
public class KleineStraat implements Observer{
	private int waarde = 0;
	private String categorieNaam = "Kleine Straat";
	...
	public void updateWaarde(ArrayList<Dobbelsteen> dobbelstenen) {
		if(this.alIngevuld == false){
			...
			if(een.size() >= 1 && twee.size() >= 1 && drie.size() >= 1 && vier.size() >= 1) {
				this.setWaarde(30);
			} else if(twee.size() >= 1 && drie.size() >= 1 && vier.size() >= 1 && vijf.size() >= 1) {
				this.setWaarde(30);
			} else if(drie.size() >= 1 && vier.size() >= 1 && vijf.size() >= 1 && zes.size() >= 1) {
				this.setWaarde(30);
			}
		}
	}
